  ...
  <div id="searchPicturePage">
    ...
    >
      <template #renderItem="{ item: picture }">
        <a-list-item style="padding: 0" v-if="!picture.loadError">
...
// 处理图片加载错误
const handleImageError = (picture: API.ImageSearchResult) => {
  picture.loadError = true;
  // 从列表中移除加载失败的图片
  dataList.value = dataList.value.filter(item => !item.loadError);
}
